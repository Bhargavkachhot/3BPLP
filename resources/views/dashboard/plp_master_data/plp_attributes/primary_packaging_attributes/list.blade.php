...
@section('title', __('Primary Packaging Attributes'))
@section('content')  
...
<div class="padding">
   <div class="box">
      ...
      <span>PLP Attributes</span> / 
         <span>Primary Packaging Attributes</span> 
      </small>
      </div> 
      {{Form::open(['route'=>['primary.packaging.attributes.bulk.action'],'method'=>'POST' ,'id'=>'bulk_action_form'])}}
      ...
      <a href="{{route('export.primary.packaging.attributes')}}"   class="btn primary m-a pull-right"><i class="fa fa-download" aria-hidden="true"></i>
      Export</a> 

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('primary.packaging.attributes.create')}}"   class="btn primary m-a pull-right"><i class="fa fa-plus" aria-hidden="true"></i>
         Create</a>
      @endif

      <div class="p-a-md">
         <h5>Primary Packaging Attributes</h5>
      </div>
      ...
      <table id="attributes" class="display" cellspacing="0" width="100%">
            <thead>
               <tr>
                  <th></th>  
                  <th>Primary Packaging Attributes</th> 
...
<script type="text/javascript">
   ...
   $.noConflict();
    ...
    'ajax': '{{route('primary.packaging.attributes.anyData')}}',
    'columnDefs': [
       {
          'targets': 0,
          'checkboxes': {
   ...
   var action = $('#action').val();  
    if(action == null){
      ...
      Swal.fire({
       ...
       Swal.fire({
       title: 'Are you sure ?',
       text: "You want to "+action+" all selected primary packaging Attributes?",
       icon: 'warning',
   ...
   Swal.fire({
   title: 'Are you sure ?',
   text: "You want to delete this primary packaging Attributes?",
   icon: 'warning',
   showCancelButton: true,
   confirmButtonColor: '#3085d6',
   cancelButtonColor: '#d33',
   ...
   var id = $(this).attr("id"); 
   $.ajax({
            url: "{{route('primary.packaging.attributes.delete')}}",
            type: 'POST',
            data: {id: id},
            error: function() {
              alert('Something is wrong, couldn\'t delete primary packaging Attributes');
            },
            success: function(data) { 
                ...
                table.ajax.reload();
                     ...
                     Swal.fire({
                        title: 'Success',
                        text: "Your primary packaging Attributes Deleted Successfully!",
                        icon: 'success', 
                      })
                }else{    
                     Swal.fire({
                        title: 'Error',
                        text: "Sorry can't delete this primary packaging Attributes cause it's assigned to child element.",
                        icon: 'error', 
                      })
                }

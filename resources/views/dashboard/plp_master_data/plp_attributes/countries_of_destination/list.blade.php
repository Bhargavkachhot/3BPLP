...
@section('title', __('Countries Of Destination'))
@section('content')  
...
<div class="padding">
   <div class="box">
      <div class="box-header dker">
         ...
         <span>Countries Of Destination</span>
         </small>
      </div> 
      {{Form::open(['route'=>['countries.of.destination.bulk.action'],'method'=>'POST' ,'id'=>'bulk_action_form'])}}
      ...
      <a href="{{route('export.countries.of.destination')}}"   class="btn primary m-a pull-right"><i class="fa fa-download" aria-hidden="true"></i>
      Export</a>

      <!-- @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('countries.of.destination.import')}}"   class="btn primary m-a pull-right"><i class="fa fa-upload" aria-hidden="true"></i>
         Import</a>
      @endif

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('countries.of.destination.create')}}"   class="btn primary m-a pull-right"><i class="fa fa-plus" aria-hidden="true"></i>
         Create</a>
      @endif -->
...
<script type="text/javascript">
   ...
   $.noConflict();
    ...
    'ajax': '{{route('countries.of.destination.anyData')}}',
    'columnDefs': [
       {
          'targets': 0,
          'checkboxes': {
   ...
   var action = $('#action').val();  
    if(action == null){
      ...
      Swal.fire({
       ...
       Swal.fire({
       title: 'Are you sure ?',
       text: "You want to "+action+" all selected Countries Of Destination?",
       icon: 'warning',
   ...
   Swal.fire({
   title: 'Are you sure ?',
   text: "You want to delete this Countries Of Destination?",
   icon: 'warning',
   showCancelButton: true,
   confirmButtonColor: '#3085d6',
   cancelButtonColor: '#d33',
   ...
   var id = $(this).attr("id"); 
   $.ajax({
            url: "{{route('countries.of.destination.delete')}}",
            type: 'POST',
            data: {id: id},
            error: function() {
              alert('Something is wrong, couldn\'t delete Countries Of Destination');
            },
            success: function(data) { 
                ...
                table.ajax.reload();
                     ...
                     Swal.fire({
                        title: 'Success',
                        text: "Your Countries Of Destination Deleted Successfully!",
                        icon: 'success', 
                      })
                }else{    
                     Swal.fire({
                        title: 'Error',
                        text: "Sorry can't delete this Countries Of Destination cause it's assigned to child element.",
                        icon: 'error', 
                      })
                }

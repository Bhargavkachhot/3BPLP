...
@section('title','Edit Countries of Origin')
@section('content')
<?php
...
<div class="box">
   <div class="box-header dker">
      <h3><i class="material-icons">&#xe3c9;</i> Edit Countries of Origin</h3>
      <small>
      <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
      <span>PLP Master Data</span> /
      <span>PLP Attributes</span> /
      <a href="{{ route('countries.of.origin') }}">Countries Of Origin</a> /
      <span>Edit Countries of Origin</span>  
      </small>
   </div> 
   <div class="box-body">
      {{Form::open(['route'=>['countries.of.origin.update'],'method'=>'POST'])}}  
      <section id="tabs">
         <div class="container">
            <div class="row">
               <div class="col-xs-12 ">
                  <input type="hidden" name="encode_id" value="{{$encode_id}}">   
                        ...
                        <div class="col-sm-12">
                           <div class="row">
                              <div class="col-sm-6">
                                 ...
                                 <input type="text" name="country_short" value="{{$countries_of_origin->country_short}}" class="form-control"> 
                              ...
                              <div class="col-sm-6">
                                 ...
                                 <input type="text" name="country" value="{{$countries_of_origin->country}}" class="form-control"> 
                                 <span class="help-block">
                              ...
                              <div class="col-sm-6">
                                 ...
                                 <input type="number" name="position" value="{{$countries_of_origin->position}}" class="form-control">
                                 <span class="help-block">
                              ...
                              <label for="Status"
                                 ...
                                 <select class="selectpicker" name="status" id="status" data-live-search="false">  
                                       <option value="1" @if($countries_of_origin->status == 1)selected @endif>Active</option>
                                       <option value="0" @if($countries_of_origin->status == 0)selected @endif>Inactive</option> 
                              </select>   
                           </div>
                           </div>
                           <br>
                           <div class="row"> 
                              ...
                              <br>
                                 <select class="tag_selection1" multiple="multiple" name="product_subcategories[]">
                                    @if(count($CountriesOfOriginSubcategory) > 0 )
                                    @foreach($CountriesOfOriginSubcategory as $key => $subcategory)
                           ...
                           <div class="row"> 
                              <div class="col-sm-12">
                                 ...
                                 <select class="tag_selection2" multiple="multiple" name="product_categories[]">
                                    @if(count($CountriesOfOriginProductCategory) > 0 )
                                    @foreach($CountriesOfOriginProductCategory as $key => $product_category)
                     ...
                     <div class="form-group row m-t-md">
                        <div class="col-sm-12">
                           ...
                           <a href="{{route('countries.of.origin')}}"
...
<script> 
   ...
   $(document).on('change', ".tag_selection1", function () {  
      ...
      $.ajax({
        url: "{{route('countries.of.origin.tags.subcategory')}}",
        type: 'POST',
        data: {
   ...
   $(document).on('change', ".tag_selection2", function () {  
      ...
      $.ajax({
        url: "{{route('countries.of.origin.tags.product.category')}}",
        type: 'POST',
        data: {

...
<div class="padding">
   <div class="box">
      ...
      {{Form::open(['route'=>['certificates.bulk.action'],'method'=>'POST' ,'id'=>'bulk_action_form'])}}
      ...
      <!-- <a href="{{route('export.certificates')}}"   class="btn primary m-a pull-right"><i class="fa fa-download" aria-hidden="true"></i>
      Export</a>

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('certificates.import')}}"   class="btn primary m-a pull-right"><i class="fa fa-upload" aria-hidden="true"></i>
         Import</a>
      @endif -->

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('certificates.create')}}"   class="btn primary m-a pull-right"><i class="fa fa-plus" aria-hidden="true"></i>
         Create</a>
      @endif
...
<script type="text/javascript">
   ...
   $.noConflict();
    ...
    'ajax': '{{route('certificates.anyData')}}',
    'columnDefs': [
       {
          'targets': 0,
          'checkboxes': {
   ...
   var action = $('#action').val();  
    if(action == null){
      ...
      Swal.fire({
       ...
       Swal.fire({
       title: 'Are you sure ?',
       text: "You want to "+action+" all selected Certificates?",
       icon: 'warning',
   ...
   Swal.fire({
   title: 'Are you sure ?',
   text: "You want to delete this Certificates?",
   icon: 'warning',
   showCancelButton: true,
   confirmButtonColor: '#3085d6',
   cancelButtonColor: '#d33',
   ...
   $.ajax({
            ...
            data: {id: id},
            error: function() {
              alert('Something is wrong, couldn\'t delete Certificates');
            },
            success: function(data) { 
                ...
                table.ajax.reload();
                     ...
                     Swal.fire({
                        title: 'Success',
                        text: "Your Certificates Deleted Successfully!",
                        icon: 'success', 
                      })
                }else{    
                     Swal.fire({
                        title: 'Error',
                        text: "Sorry can't delete this Certificates cause it's assigned to child element.",
                        icon: 'error', 
                      })
                }

...
@section('title', __('Primary categories'))
@section('content')  
<?php
...
<div class="padding">
   <div class="box">
      ...
      {{Form::open(['route'=>['primary.categories.bulk.action'],'method'=>'POST' ,'id'=>'bulk_action_form'])}}
      ...
      @endif
      <a href="{{route('export.primary.categories')}}"   class="btn primary m-a pull-right"><i class="fa fa-download" aria-hidden="true"></i>
      Export</a>
      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
      <a href="{{route('primary.categories.create')}}" class="btn primary m-a pull-right"><i class="fa fa-plus" aria-hidden="true"></i>
      Create</a>
      @endif  
      <div class="p-a-md">
...
<script type="text/javascript">
   ...
   $.noConflict();
    ...
    'ajax': '{{route('primary.categories.anyData')}}',
    'columnDefs': [
       {
          'targets': 0,
          'checkboxes': {
   ...
   var id = $(this).attr("id"); 
   $.ajax({
            url: "{{route('primary.categories.delete')}}",
            type: 'POST',
            data: {id: id},
            error: function() {
            ...
            success: function(data) { 
                ...
                table.ajax.reload();
                     ...
                     Swal.fire({
                        title: 'Error',
                        text: "Sorry can't delete this category cause it's assigned to subcategories, product categories and example products.",
                        icon: 'error', 
                      })
                }

...
@section('title','Edit SKU Packaging')
@section('content')
<?php
...
<div class="box">
   <div class="box-header dker">
      <h3><i class="material-icons">&#xe3c9;</i> Edit SKU Packaging</h3>
      <small>
      <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
      <span>PLP Master Data</span> /
      <span>PLP Attributes</span> /
     <a href="{{ route('sku.packaging') }}">SKU Packaging</a> /
      <span>Edit SKU Packaging</span>  
      </small>
   </div> 
   <div class="box-body">
      {{Form::open(['route'=>['sku.packaging.update'],'method'=>'POST'])}}  
      <section id="tabs">
         <div class="container">
            <div class="row">
               <div class="col-xs-12 ">
                           ...
                           <div class="row">
                              <div class="col-sm-6">
                                 <label for="category_name"
                                    class="form-control-label">SKU Packaging <span class="valid_field">*</span> 
                                 </label> 
                                 <input type="text" name="sku_packaging" value="{{$sku_packaging->sku_packaging}}" class="form-control"> 
                                 <span class="help-block">
                                 @if(!empty(@$errors) && @$errors->has('sku_packaging'))
                                 <span  style="color: red;" class='validate'>{{ $errors->first('sku_packaging') }}</span>
                                 @endif
                                 </span> 
                              </div>
                              <div class="col-sm-6">
                                 ...
                                 <input type="number" name="position" value="{{$sku_packaging->position}}" class="form-control">
                                 <span class="help-block">
                              ...
                              <label for="Status"
                                 ...
                                 <select class="selectpicker" name="status" id="status" data-live-search="false">  
                                       <option value="1" @if($sku_packaging->status == 1)selected @endif>Active</option>
                                       <option value="0" @if($sku_packaging->status == 0)selected @endif>Inactive</option> 
                              </select>   
                           </div>
                           </div>
                           <br>
                           <div class="row"> 
                              ...
                              <br>
                                 <select class="tag_selection1" multiple="multiple" name="product_subcategories[]">
                                    @if(count($SkuPackagingSubcategory) > 0 )
                                    @foreach($SkuPackagingSubcategory as $key => $subcategory)
                           ...
                           <div class="row"> 
                              <div class="col-sm-12">
                                 ...
                                 <select class="tag_selection2" multiple="multiple" name="product_categories[]">
                                    @if(count($SkuPackagingCategory) > 0 )
                                    @foreach($SkuPackagingCategory as $key => $product_category)
...
<script> 
   ...
   $(document).on('change', ".tag_selection1", function () {  
      ...
      $.ajax({
        url: "{{route('sku.packaging.tags.subcategory')}}",
        type: 'POST',
        data: {
   ...
   $(document).on('change', ".tag_selection2", function () {  
      ...
      $.ajax({
        url: "{{route('sku.packaging.tags.product.category')}}",
        type: 'POST',
        data: {

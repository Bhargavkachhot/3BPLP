...
<div class="box">
   <div class="box-header dker">
      <h3><i class="material-icons">&#xe3c9;</i> Edit Product Attribute</h3>
      <small>
      <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
      <span>PLP Master Data</span> /
      <span>PLP Attributes</span> /
      <a href="{{ route('product.attributes') }}">Product Attributes</a> /
      <span>Edit Product Attribute</span>  
      </small>
   </div> 
   <div class="box-body">
      {{Form::open(['route'=>['product.attributes.update'],'method'=>'POST'])}}  
      <section id="tabs">
         <div class="container">
            <div class="row">
               <div class="col-xs-12 ">
                  <input type="hidden" name="encode_id" value="{{$encode_id}}">   
                           ...
                           <div class="row">
                              <div class="col-sm-6">
                                 <label for="category_name"
                                    class="form-control-label">Product Attribute <span class="valid_field">*</span>   
                                 </label> 
                                 <input type="text" name="product_attribute" value="{{$product_attributes->product_attribute}}" class="form-control"> 
                                 <span class="help-block">
                                 @if(!empty(@$errors) && @$errors->has('product_attribute'))
                                 <span  style="color: red;" class='validate'>{{ $errors->first('product_attribute') }}</span>
                                 @endif
                                 </span> 
                              </div>
                              <div class="col-sm-6">
                                 ...
                                 <input type="number" name="position" min="1"  onkeydown="javascript: return event.keyCode == 69 ? false : true" value="{{$product_attributes->position}}" class="form-control">
                                 <span class="help-block">
                              ...
                              <label for="Status"
                                 ...
                                 <select class="selectpicker" name="status" id="status" data-live-search="false">  
                                       <option value="1" @if($product_attributes->status == 1)selected @endif>Active</option>
                                       <option value="0" @if($product_attributes->status == 0)selected @endif>Inactive</option> 
                              </select>   
                           </div>
                           </div>
                           <br>
                           <div class="row"> 
                              ...
                              <br>
                                 <select class="tag_selection1" multiple="multiple" name="product_subcategories[]">
                                    @if(count($ProductAttributeSubcategory) > 0 )
                                    @foreach($ProductAttributeSubcategory as $key => $subcategory)
                           ...
                           <div class="row"> 
                              <div class="col-sm-12">
                                 ...
                                 <select class="tag_selection2" multiple="multiple" name="product_categories[]">
                                    @if(count($ProductAttributeCategory) > 0 )
                                    @foreach($ProductAttributeCategory as $key => $product_category)
                     ...
                     <div class="form-group row m-t-md">
                        <div class="col-sm-12">
                           ...
                           <a href="{{route('product.attributes')}}"
...
<script> 
   ...
   $(document).on('change', ".tag_selection1", function () {  
      ...
      $.ajax({
        url: "{{route('product.attributes.tags.subcategory')}}",
        type: 'POST',
        data: {
   ...
   $(document).on('change', ".tag_selection2", function () {  
      ...
      $.ajax({
        url: "{{route('product.attributes.tags.product.category')}}",
        type: 'POST',
        data: {

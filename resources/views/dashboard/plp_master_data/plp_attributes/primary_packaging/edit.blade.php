...
@section('title','Edit Primary Packaging')
@section('content')
<?php
...
<div class="box">
   <div class="box-header dker">
      <h3><i class="material-icons">&#xe3c9;</i> Edit Primary Packaging</h3>
      <small>
      <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
      <span>PLP Master Data</span> /
      <span>PLP Attributes</span> /
      <a href="{{ route('primary.packaging') }}">Primary Packaging</a> /
      <span>Edit Primary Packaging</span>  
      </small>
   </div> 
   <div class="box-body">
      {{Form::open(['route'=>['primary.packaging.update'],'method'=>'POST'])}}  
      <section id="tabs">
         <div class="container">
            <div class="row">
               <div class="col-xs-12 ">
                        ...
                        <div class="col-sm-12">
                           <div class="row">
                              <div class="col-sm-6">
                                 ...
                                 <input type="text" name="primary_packaging" value="{{$primary_packaging->primary_packaging}}" class="form-control"> 
                              ...
                              <div class="col-sm-6">
                                 ...
                                 <input type="number" name="position" value="{{$primary_packaging->position}}" class="form-control">
                                 <span class="help-block">
                              ...
                              <label for="Status"
                                 ...
                                 <select class="selectpicker" name="status" id="status" data-live-search="false">  
                                       <option value="1" @if($primary_packaging->status == 1)selected @endif>Active</option>
                                       <option value="0" @if($primary_packaging->status == 0)selected @endif>Inactive</option> 
                              </select>   
                           </div>
                           </div>
                           <br>
                           <div class="row"> 
                              ...
                              <br>
                                 <select class="tag_selection1" multiple="multiple" name="product_subcategories[]">
                                    @if(count($primaryPackagingSubcategory) > 0 )
                                    @foreach($primaryPackagingSubcategory as $key => $subcategory)
                           ...
                           <div class="row"> 
                              <div class="col-sm-12">
                                 ...
                                 <select class="tag_selection2" multiple="multiple" name="product_categories[]">
                                    @if(count($primaryPackagingCategory) > 0 )
                                    @foreach($primaryPackagingCategory as $key => $product_category)
...
<script> 
   ...
   $(document).on('change', ".tag_selection1", function () {  
      ...
      $.ajax({
        url: "{{route('primary.packaging.tags.subcategory')}}",
        type: 'POST',
        data: {
   ...
   $(document).on('change', ".tag_selection2", function () {  
      ...
      $.ajax({
        url: "{{route('primary.packaging.tags.product.category')}}",
        type: 'POST',
        data: {

...
@section('title', __('SKU Packaging'))
@section('content')  
...
<div class="padding">
   <div class="box">
      ...
      <span>PLP Attributes</span> / 
         <span>SKU Packaging</span>
         </small>
      </div> 
      {{Form::open(['route'=>['sku.packaging.bulk.action'],'method'=>'POST' ,'id'=>'bulk_action_form'])}}
      ...
      <!-- <a href="{{route('export.sku.packaging')}}"   class="btn primary m-a pull-right"><i class="fa fa-download" aria-hidden="true"></i>
      Export</a>

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('sku.packaging.import')}}"   class="btn primary m-a pull-right"><i class="fa fa-upload" aria-hidden="true"></i>
         Import</a>
      @endif -->

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('sku.packaging.create')}}"   class="btn primary m-a pull-right"><i class="fa fa-plus" aria-hidden="true"></i>
         Create</a>
      @endif
...
<script type="text/javascript">
   ...
   $.noConflict();
    ...
    'ajax': '{{route('sku.packaging.anyData')}}',
    'columnDefs': [
       {
          'targets': 0,
          'checkboxes': {
   ...
   var action = $('#action').val();  
    if(action == null){
      ...
      Swal.fire({
       ...
       Swal.fire({
       title: 'Are you sure ?',
       text: "You want to "+action+" all selected SKU packaging?",
       icon: 'warning',
   ...
   Swal.fire({
   title: 'Are you sure ?',
   text: "You want to delete this SKU packaging?",
   icon: 'warning',
   showCancelButton: true,
   confirmButtonColor: '#3085d6',
   cancelButtonColor: '#d33',
   ...
   var id = $(this).attr("id"); 
   $.ajax({
            url: "{{route('sku.packaging.delete')}}",
            type: 'POST',
            data: {id: id},
            error: function() {
              alert('Something is wrong, couldn\'t delete SKU packaging');
            },
            success: function(data) { 
                ...
                table.ajax.reload();
                     ...
                     Swal.fire({
                        title: 'Success',
                        text: "Your SKU packaging Deleted Successfully!",
                        icon: 'success', 
                      })
                }else{    
                     Swal.fire({
                        title: 'Error',
                        text: "Sorry can't delete this SKU packaging cause it's assigned to child element.",
                        icon: 'error', 
                      })
                }

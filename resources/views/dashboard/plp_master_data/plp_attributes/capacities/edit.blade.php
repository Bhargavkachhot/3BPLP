...
<div class="box">
   <div class="box-header dker">
      <h3><i class="material-icons">&#xe3c9;</i> Edit Capacity</h3>
      <small>
      <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
      <span>PLP Master Data</span> /
      <span>PLP Attributes</span> /
      <a href="{{ route('capacities') }}">Capacity</a> /
      <span>Edit Capacity</span>  
      </small>
   </div> 
   <div class="box-body">
      {{Form::open(['route'=>['capacities.update'],'method'=>'POST'])}}  
      <section id="tabs">
         <div class="container">
            <div class="row">
               <div class="col-xs-12 ">
                           ...
                           <div class="row">  
                              <div class="col-sm-6">
                                 <label for="category_name"
                                    class="form-control-label">Capacity <span class="valid_field">*</span>
                                 </label> 
                                 <input type="text" name="capacity" value="{{$capacity->capacity}}" class="form-control"> 
                                 <span class="help-block">
                                 @if(!empty(@$errors) && @$errors->has('capacity'))
                                 <span  style="color: red;" class='validate'>{{ $errors->first('capacity') }}</span>
                                 @endif
                                 </span> 
                              </div>
                              <div class="col-sm-6">
                                 ...
                                 <input type="number" name="position" value="{{$capacity->position}}" class="form-control">
                                 <span class="help-block">
                              ...
                              <label for="Status"
                                 ...
                                 <select class="selectpicker" name="status" id="status" data-live-search="false">  
                                       <option value="1" @if($capacity->status == 1)selected @endif>Active</option>
                                       <option value="0" @if($capacity->status == 0)selected @endif>Inactive</option> 
                              </select> 
                           </div>
                           </div>
                           <br>
                           <div class="row"> 
                              ...
                              <br>
                                 <select class="tag_selection1" multiple="multiple" name="product_subcategories[]">
                                    @if(count($CapacitySubcategory) > 0 )
                                    @foreach($CapacitySubcategory as $key => $subcategory)
                           ...
                           <div class="row"> 
                              <div class="col-sm-12">
                                 ...
                                 <select class="tag_selection2" multiple="multiple" name="product_categories[]">
                                    @if(count($CapacityProductCategory) > 0 )
                                    @foreach($CapacityProductCategory as $key => $product_category)
...
<script> 
   ...
   $(document).on('change', ".tag_selection1", function () {  
      ...
      $.ajax({
        url: "{{route('capacities.tags.subcategory')}}",
        type: 'POST',
        data: {
   ...
   $(document).on('change', ".tag_selection2", function () {  
      ...
      $.ajax({
        url: "{{route('capacities.tags.product.category')}}",
        type: 'POST',
        data: {

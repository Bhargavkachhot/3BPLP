...
<div class="padding">
   <div class="box">
      ...
      {{Form::open(['route'=>['subcategories.bulk.action'],'method'=>'POST' ,'id'=>'bulk_action_form'])}}
      ...
      @if(isset($allowed_permissions) && $allowed_permissions->delete == 1)
         <div class="bulk-action">
            <select name="action" id="action" class="form-control c-select w-sm inline v-middle" required>
               <option value="null" disabled selected>Bulk action</option>
      ...
      <a href="{{route('export.subcategories')}}"   class="btn primary m-a pull-right"><i class="fa fa-download" aria-hidden="true"></i>
      Export</a>

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('subcategories.create')}}"   class="btn primary m-a pull-right"><i class="fa fa-plus" aria-hidden="true"></i>
         Create</a>
      @endif 
         ...
         <table id="categories" class="display" cellspacing="0" width="100%">
            <thead>
               <tr>
                  ...
                  <th>Subcategory</th>
                  <th data-orderable="false">URL key</th> 
                  <th data-orderable="false">Full URL key</th>
                  <th>Position</th>
                  <th data-orderable="false">Status</th>
                  <th data-orderable="false" width="100px;">options</th>
               </tr>
            </thead>
         </table>
...
<script type="text/javascript">
   ...
   $.noConflict();
    ...
    'ajax': '{{route('subcategories.anyData')}}',
    'columnDefs': [
       {
          'targets': 0,
          'checkboxes': {
   ...
   $.ajax({
            ...
            success: function(data) { 
                ...
                table.ajax.reload();
                     ...
                     Swal.fire({
                        title: 'Error',
                        text: "Sorry can't delete this category cause it's assigned to product categories, example products, product attributes and primary packaging.",
                        icon: 'error', 
                      })
                }

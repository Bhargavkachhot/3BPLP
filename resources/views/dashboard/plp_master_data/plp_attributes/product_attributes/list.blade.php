...
<div class="padding">
   <div class="box">
      ...
      {{Form::open(['route'=>['product.attributes.bulk.action'],'method'=>'POST' ,'id'=>'bulk_action_form'])}}
      ...
      <a href="{{route('export.product.attributes')}}"   class="btn primary m-a pull-right"><i class="fa fa-download" aria-hidden="true"></i>
      Export</a>

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('product.attributes.import')}}"   class="btn primary m-a pull-right"><i class="fa fa-upload" aria-hidden="true"></i>
         Import</a>
      @endif

      @if(isset($allowed_permissions) && $allowed_permissions->create == 1)
         <a href="{{route('product.attributes.create')}}"   class="btn primary m-a pull-right"><i class="fa fa-plus" aria-hidden="true"></i>
         Create</a>
      @endif
...
<script type="text/javascript">
   ...
   $.noConflict();
    ...
    'ajax': '{{route('product.attributes.anyData')}}',
    'columnDefs': [
       {
          'targets': 0,
          'checkboxes': {
   ...
   var action = $('#action').val();  
    if(action == null){
      ...
      Swal.fire({
       ...
       Swal.fire({
       title: 'Are you sure ?',
       text: "You want to "+action+" all selected attribute?",
       icon: 'warning',
   ...
   $.ajax({
            ...
            data: {id: id},
            error: function() {
              alert('Something is wrong, couldn\'t delete attribute');
            },
            success: function(data) { 
                ...
                table.ajax.reload();
                     ...
                     Swal.fire({
                        title: 'Success',
                        text: "Your attribute Deleted Successfully!",
                        icon: 'success', 
                      })
                }else{    
                     Swal.fire({
                        title: 'Error',
                        text: "Sorry can't delete this attribute cause it's assigned to subcategories, product categories and example products.",
                        icon: 'error', 
                      })
                }

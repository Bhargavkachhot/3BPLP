...
class ImportCapacity implements ToCollection, WithStartRow,WithValidation

{
    ...
    public function rules(): array
    {
        return [
            '1' => 'required|max:50', 
            '2' => 'required|unique:capacities,position,NULL,id,status,0|unique:capacities,position,NULL,id,status,1',  
        ]; 
    }
    ...
    {
        return [ 
            '1.required'    => 'capacity is required.', 
            '1.max'         => 'The maximun length of the capacity must not exceed :max',
        ...
        if($subcategory['all_matched'] == true && $product_category['all_matched'] == true){
            ...
            {  
                ...
                if(count($product_category_id) > 0 && count($subcategory_id) > 0){
                        $create_PrimaryPackaging = Capacity::create([
                           'capacity' => $row[1], 
                           'position' => $row[2],
                           'status' => 1,
                        ]); 
                    foreach ($subcategory_id as $key => $value) {
                         $create_subcategory = CapacitySubcategory::create([
                           'capacity_id' => $create_PrimaryPackaging->id,
                           'subcategory_id' => $value, 
                        ]);
                    }

                    foreach ($product_category_id as $key => $value) {
                        $create_product_category = CapacityProductCategory::create([
                           'capacity_id' => $create_PrimaryPackaging->id,
            ...
            return redirect()->route('capacities')->with('success', 'Data imported successfully.'); 
        }else{ 
